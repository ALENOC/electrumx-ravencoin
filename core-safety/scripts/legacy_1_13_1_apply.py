...
The 1.13.6 transactional updater normally requires the installer marker and
...
write the adoption marker until the already-discovered pending 1.13.6 candidate
...
TARGET_ELECTRUMX_VERSION = "1.13.6"
...
def _prove_candidate_named_storage(
    ...
    # staged marker after storage proof so the activated 1.13.6 root retains
...
def _preflight_pending_candidate() -> dict:
    """Fully authenticate the pending 1.13.6 candidate before marker mutation."""
    try:
    ...
    if not state.pending_candidate:
        raise LegacyAdoptionError(
            "no pending candidate; run electrumx-update check after the signed 1.13.6 release exists")
