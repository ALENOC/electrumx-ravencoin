...
"""Render the standalone 1.13.2 installer from the reviewed source template.

The checked-in installer remains the historical v1-schema bootstrap template so
1.13.1 behavior is explicit and reviewable. Release 1.13.2 is rendered with
...
def render(*, output: pathlib.Path, public_key_hex: str) -> None:
    ...
    if public_key_hex == RETIRED_UPDATE_PUBLIC_KEY_HEX:
        raise RenderError("retired CI-held update key is forbidden for 1.13.2")
    if hashlib.sha256(bytes.fromhex(public_key_hex)).hexdigest()[:16] == RETIRED_UPDATE_KEY_ID:
        raise RenderError("retired update key id is forbidden for 1.13.2")
    ...
    source = _replace_once(
        ...
        '    parser.add_argument(\n'
        '        "--qualification-candidate-dir", default=None, metavar="DIR",\n'
        '        help="PRE-PUBLICATION QUALIFICATION: read signed candidate manifest/bundle "\n'
        '             "locally while keeping the embedded production trust roots")\n'
        '    parser.add_argument(\n'
        '        "--storage-root", default=None, metavar="DIR",\n',
    ...
    source = _replace_once(
        ...
        '    args = parser.parse_args(argv)\n'
        '    if args.local_release_validation_dir and args.qualification_candidate_dir:\n'
        '        parser.error("local validation and qualification candidate modes are mutually exclusive")\n'
        '    if args.chainstrap and args.p2p_bootstrap:\n',
        "qualification argument exclusion")
    source = _replace_once(
        source,
        ...
        '            public_key_hex, core_policy_public_key_hex, manifest_fetch, bundle_fetch = \\\n'
        '                load_local_release_validation(validation_dir)\n'
        '        elif args.qualification_candidate_dir:\n'
        '            qualification_dir = Path(args.qualification_candidate_dir)\n'
        '            print_qualification_banner(qualification_dir)\n'
        '            public_key_hex, core_policy_public_key_hex, manifest_fetch, bundle_fetch = \\\n'
        '                load_qualification_candidate(qualification_dir)\n'
        '        else:\n'
        '            public_key_hex = RELEASE_PUBLIC_KEY_HEX\n'
        '            core_policy_public_key_hex = PRODUCTION_CORE_POLICY_PUBLIC_KEY_HEX\n'
        '            manifest_fetch = None\n'
        '            bundle_fetch = None\n',
        "qualification transport selection")
    source = _replace_once(
