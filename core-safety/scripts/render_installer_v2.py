...
"""Render the standalone 1.13.3 installer from the reviewed source template.

The checked-in installer remains the historical v1-schema bootstrap template so
1.13.1 behavior is explicit and reviewable. Release 1.13.3 is rendered with
...
def render(*, output: pathlib.Path, public_key_hex: str) -> None:
    ...
    if public_key_hex == RETIRED_UPDATE_PUBLIC_KEY_HEX:
        raise RenderError("retired CI-held update key is forbidden for 1.13.3")
    if hashlib.sha256(bytes.fromhex(public_key_hex)).hexdigest()[:16] == RETIRED_UPDATE_KEY_ID:
        raise RenderError("retired update key id is forbidden for 1.13.3")
    ...
    source = _replace_once(
        ...
        '"alenoc/electrumx-ravencoin:1.13.3", "-ec",', "release image version")
    source = _replace_once(
