...
* `network_observer/crawl.py:288-342` `Crawler.crawl()`: breadth-first crawl over
...
* `network_observer/netsafety.py:183-206` `parse_peers_response()`: bounded peer
  list parsing (`max_peers_per_response`), junk-tolerant per-record parse.
* Seeds and registry are hints only: `network_observer/cli.py:80-113`
  `load_seeds()` / `load_registry()`; the config files themselves state
  "a seed is a hint, never an endorsement"
  (`network_observer/config/seeds.json`).
* Discovery state machine: `network_observer/model.py:60-64`
  `DiscoverySource` (BOOTSTRAP, GOSSIP, REGISTRY, MANUAL).

## B. Existing network-safety protections

* `network_observer/netsafety.py:48-81` `normalize_hostname()`: RFC 1123 label
  validation, length cap, separator/host-string attacks, onion handling,
  bare-IP classification.
* `network_observer/netsafety.py:95-116` `classify_address()`: refuses loopback,
  ...
  37-41.
* `network_observer/netsafety.py:119-138` `safe_resolved_addresses()`: filters
  every DNS answer, not just the first, which is the DNS-rebinding shape.
* `network_observer/crawl.py:72-100` `resolve_endpoint()`: DNS timeout, refuses
  `.onion` rather than leaking it to the system resolver.
* Response amplification bounds: `network_observer/model.py:147-162` `Limits`
  (`max_response_bytes`, `max_peers_per_response`,
  `max_probes_per_host_per_hour`, `max_concurrent_probes`);
  `network_observer/crawl.py:275-276` enforces the per-response size limit.

## C. Existing backend/Core trust model

* `network_observer/classify.py:85-138` `classify_backend()`: a self-reported
...
* `network_observer/cli.py:119-146` `load_policy()`: verification failure of the
  signed policy collapses to an empty policy (everything
  UNREVIEWED_CORE), never to trust.
* Anti-rollback for policy versions: `network_observer/store.py:326-344`
...
* `network_observer/classify.py:44-56` `operator_group_key()` and
  ...
  free.
* `network_observer/classify.py:159-170` `independent_groups()`.
* `network_observer/classify.py:320-341` `count_independent_operators()`: counts
  attested groups, not endpoints.
* Grouping only ever comes from this operator's own seeds/registry config
  (`network_observer/config/operator-registry.json`), never from crawled
  self-reports.

## E. Existing chain comparison behavior

* `network_observer/classify.py:224-301` `compare_chains()`: operator-group
...
* Corroboration: `network_observer/classify.py:173-221` `_verified_groups()`
...
* `network_observer/classify.py:304-317` `is_corroborated()`: SAFE promotion
  requires positive verification; absence of conflict is not agreement.
* Header hashing: `network_observer/crawl.py:103-125`
...
* `network_observer/store.py:347-388`: `record_conflict()`, `clear_conflict()`,
  ...
  a fresh conflict (`network_observer/cli.py:213-218`).

## G. Existing signed directory architecture

* `network_observer/directory.py:26-27` schema v1 with signature domain
...
* `network_observer/model.py:170` `ProbeResult.vantage_point` (default "local");
  persisted per observation (`network_observer/store.py:63` column
  `vantage_point`, recorded at `store.py:278`); CLI flag
  `--vantage-point` (`network_observer/cli.py:317-319`). It is a label today:
...
4. `blockchain.block.header` is already used once per probe for the
   pinned incident checkpoint (`network_observer/crawl.py:50`); the shared-height
